...
Ce document consigne les règles de monétisation et d'intégration publicitaire obligatoires pour chaque nouvel article créé sur **parisfenetres.com**.

---

## 1. Principe Général : Zéro ID en Dur

La monétisation de ce site s'appuie sur le serveur autonome **Revive Adserver** (`https://ads.les4h.fr`, Éditeur ID `115`).
Tous les identifiants de zones Revive attribués à **parisfenetres.com** sont centralisés dans le fichier de configuration :
👉 **`src/config/ads.config.ts`**

> **Règle absolue** : Il est strictement interdit d'écrire des numéros de zone Revive (ex: `zoneId={691}`) directement dans les articles Markdown, les templates Astro ou les composants. Tout appel d'emplacement publicitaire doit obligatoirement se faire via son identifiant sémantique (`slot`).
...
| :--- | :--- | :---: | :--- |
| **`header`** | Leaderboard (728x90) | Zone 690 | Bannière haute injectée au-dessus ou sous le menu principal |
| **`inContent`** | Medium Rectangle (300x250) | Zone 691 | Encart contextuel inséré au milieu de l'article |
| **`largeRectangle`** | Large Rectangle (336x280) | Zone 689 | Encart de fin d'article inséré avant les articles recommandés |
| **`mobileSticky`** | Mobile Banner (320x100) | Zone 692 | Bandeau collant ancré en bas d'écran sur smartphone |
| **`halfPage`** | Half Page (300x600) | Zone 688 | Grand format pour colonne latérale (Sidebar desktop) |
| **`skyscraper`** | Skyscraper (160x600) | Zone 693 | Format vertical pour listing et dossiers thématiques |
