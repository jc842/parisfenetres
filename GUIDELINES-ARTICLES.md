# Directives Éditoriales & Contraintes d'Environnement — parisfenetres.com

Ce document récapitule les directives d'environnement et de monétisation obligatoires pour toute création de nouvel article sur **parisfenetres.com**.
...
| :--- | :--- |
| **Domaine** | `parisfenetres.com` |
| **Tracker Analytics** | Matomo cookieless (`https://analytics.les4h.fr/`) |
| **Conteneur MTM** | `container_cJnAXfBX.js` (Site ID: `62`) |
| **Adserver Revive** | `https://ads.les4h.fr/` (Éditeur ID: `115`) |
...
```html
<script type="text/javascript" src="https://analytics.les4h.fr/js/container_cJnAXfBX.js" async defer></script>
```
...
<!-- 1. Encart Pub In-Content Obligatoire (Medium Rectangle 300x250 - Zone 691) -->
<div class="my-12 py-6 border-y border-stone-200/80 flex justify-center">
  <ReviveSlot slot="inContent" />
</div>

<!-- 2. Encart Pub Fin d'Article Obligatoire (Large Rectangle 336x280 - Zone 689) -->
...
| :--- | :--- | :---: | :--- |
| **`header`** | Leaderboard (728x90) | **690** | Haut de page / sous navigation |
| **`inContent`** | Medium Rectangle (300x250) | **691** | Milieu d'article contextuel |
| **`largeRectangle`** | Large Rectangle (336x280) | **689** | Fin d'article avant recommandations |
| **`mobileSticky`** | Mobile Banner (320x100) | **692** | Bandeau sticky bas d'écran mobile |
| **`halfPage`** | Half Page (300x600) | **688** | Sidebar desktop pour articles longs |
| **`skyscraper`** | Skyscraper (160x600) | **693** | Colonne latérale de listing / dossiers |

---

## 4. Checklist Avant Publication
- [ ] L'article utilise le Layout avec le tracker MTM actif (`cJnAXfBX`).
