# Directives & Contraintes d'Environnement — parisfenetres.com

Ce document consigne l'architecture technique, les contraintes d'environnement, les directives de sécurité et les règles éditoriales pour le site **parisfenetres.com** (parisfenetres) hébergé sur Cloudflare Pages.
...
| :--- | :--- |
| **Domaine Apex** | `parisfenetres.com` |
| **URL Cloudflare Pages** | `https://parisfenetres.pages.dev` |
| **Dépôt GitHub** | `jc842/parisfenetres` (branche principale : `main`) |
...
| **Matomo Site ID** | `62` |
| **Matomo MTM Container** | `container_cJnAXfBX.js` (Container ID : `cJnAXfBX`) |
| **Serveur Publicitaire** | Revive Adserver (`https://ads.les4h.fr/`) |
| **Revive Affiliate ID** | `115` |
| **Zones Publicitaires** | Header (`690`), In-Content (`691`), Mobile Sticky (`692`), Large Rectangle (`689`), Half Page (`688`), Skyscraper (`693`) |
...
- Ce Layout injecte au build statique le composant `Matomo.astro` avec le conteneur MTM dédié : **`container_cJnAXfBX.js`**.
...
- Tout article doit obligatoirement comporter les 2 emplacements publicitaires intégrés dans le gabarit (`src/pages/blog/[...slug].astro`) :
  1. **Encart In-Content (300x250)** : `<ReviveSlot slot="inContent" />` positionné après les premiers paragraphes (Zone 691).
  2. **Encart Fin d'Article (336x280)** : `<ReviveSlot slot="largeRectangle" />` positionné avant les suggestions de maillage (Zone 689).
